...
public class CollectionShelfService {
    ...
    public CompletableFuture<Void> addShelfAsync(Long userId, Long collectionId, Long shelfId) {
        try {
            Collection collection = collectionRepository.findByIdAndUserIdWithShelves(collectionId, userId)
            ...
            boolean alreadyExists = collection.getShelves().stream()
            ...
            log.warn("Falha ao adicionar estante {} à coleção {} (usuário {}): {}",
                    shelfId, collectionId, userId, e.getMessage());
        }
    ...
    public CompletableFuture<Void> removeShelfAsync(Long userId, Long collectionId, Long shelfId) {
        try {
            ...
            log.warn("Falha ao remover estante {} da coleção {} (usuário {}): {}",
                    shelfId, collectionId, userId, e.getMessage());
        }
        return CompletableFuture.completedFuture(null);
    }
