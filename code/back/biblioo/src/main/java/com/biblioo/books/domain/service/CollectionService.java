...
public class CollectionService implements CollectionUseCase {
    ...
    public Collection getCollection(Long userId, Long collectionId) {
        return collectionRepository.findByIdAndUserIdWithShelves(collectionId, userId)
                .orElseThrow(() -> new ShelfBusinessException("Coleção não encontrada ou não pertence ao usuário."));
    }

    @Override
    @Transactional
    @CacheEvict(value = "collection-list", key = "#userId")
    public Collection createCollection(Long userId, String name, String description, List<Long> initialShelfIds) {
        validateName(name);
    ...
    public Collection updateCollection(Long userId, Long collectionId, String name, String description) {
    ...
    private List<Shelf> resolveShelvesForUser(List<Long> shelfIds, Long userId) {
        ...
        if (shelves.size() != shelfIds.size()) {
            throw new ShelfBusinessException(
                    "Uma ou mais estantes informadas não foram encontradas.");
        }
