...
export function setup() {
  ...
  for (let i = 0; i < 5; i++) {
    const comm = http.post(
      ...
      JSON.stringify({
        name: `Trending Test Comm ${ownerTs}_${i}`,
        description: 'Comunidade gerada para testes de trending',
        type: 'PUBLIC',
        bookId: CONFIG.communityBookIds[i % CONFIG.communityBookIds.length],
      }),
      ownerAuth
    );
    check(comm, { 'create community 201': (r) => r.status === 201 });
...
export const options = {
  ...
  thresholds: {
    ...
    'http_req_duration{name:GET_trending_communities}':   [`p(95)<${CONFIG.thresholds.p95}`],
    'http_req_duration{name:GET_trending_books}':         [`p(95)<${CONFIG.thresholds.p95}`],
...
export default function (data) {
  ...
  const commOk = check(commRes, {
    'GET /trending/communities 200': (r) => r.status === 200,
    'trending/communities é array':  (r) => Array.isArray(r.json()),
  });
  ...
  const booksOk = check(booksRes, {
    'GET /trending/books 200':  (r) => r.status === 200,
    'trending/books é array':   (r) => Array.isArray(r.json()),
  });
