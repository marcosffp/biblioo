...
export function setup() {
  ...
  for (let i = 0; i < 5; i++) {
    const comm = http.post(
      ...
      JSON.stringify({
        ...
        description: 'Comunidade para teste de spike',
        type: 'PUBLIC',
  ...
  for (let i = 0; i < CONFIG.poolSize; i++) {
    ...
    const shelf = http.post(`${CONFIG.baseUrl}/shelves`, JSON.stringify({ name: `Estante ${i}` }), authJson);
    ...
    const rvBody = multipart({ bookId, rating: (i % 5) + 1, text: `Review spike ${i}` });
