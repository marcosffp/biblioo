import 'package:biblioo/core/di/injector.dart';
import 'package:biblioo/features/book/bloc/book_bloc.dart';
import 'package:biblioo/features/book/bloc/book_event.dart';
import 'package:biblioo/features/book/bloc/book_state.dart';
import 'package:biblioo/features/book/domain/book.dart';
import 'package:biblioo/features/community/bloc/community_voting_bloc.dart';
import 'package:biblioo/features/community/bloc/community_voting_event.dart';
import 'package:biblioo/features/community/domain/book_voting.dart';
import 'package:flutter/material.dart';
import 'package:flutter_bloc/flutter_bloc.dart';

class CreateVotingSheet extends StatefulWidget {
  final int communityId;

  const CreateVotingSheet({super.key, required this.communityId});

  static void show(BuildContext context, int communityId) {
    final votingBloc = context.read<CommunityVotingBloc>();
    showModalBottomSheet(
      context: context,
      isScrollControlled: true,
      useSafeArea: true,
      shape: const RoundedRectangleBorder(
        borderRadius: BorderRadius.vertical(top: Radius.circular(20)),
      ),
      builder: (_) => MultiBlocProvider(
        providers: [
          BlocProvider.value(value: votingBloc),
          BlocProvider(create: (_) => BookBloc(Injector.instance.bookRepo)),
        ],
        child: CreateVotingSheet(communityId: communityId),
      ),
    );
  }

  @override
  State<CreateVotingSheet> createState() => _CreateVotingSheetState();
}

class _CreateVotingSheetState extends State<CreateVotingSheet> {
  final _formKey = GlobalKey<FormState>();
  final _titleController = TextEditingController();
  final _bookSearchController = TextEditingController();

  final List<Book> _selectedBooks = [];
  TieBreakRule _tieBreakRule = TieBreakRule.adminChoice;

  @override
  void dispose() {
    _titleController.dispose();
    _bookSearchController.dispose();
    super.dispose();
  }

  void _onBookSearchChanged(String query) {
    if (query.isEmpty) {
      context.read<BookBloc>().add(BookSearchCleared());
    } else {
      context.read<BookBloc>().add(BookSearchRequested(query));
    }
  }

  void _selectBook(Book book) {
    if (_selectedBooks.any((item) => item.id == book.id)) return;
    if (_selectedBooks.length >= 6) return;

    setState(() {
      _selectedBooks.add(book);
      _bookSearchController.clear();
    });
    context.read<BookBloc>().add(BookSearchCleared());
  }

  void _removeBook(Book book) {
    setState(() => _selectedBooks.removeWhere((item) => item.id == book.id));
  }

  void _submit() {
    if (!_formKey.currentState!.validate()) return;
    if (_selectedBooks.length < 3 || _selectedBooks.length > 6) return;

    final now = DateTime.now();
    context.read<CommunityVotingBloc>().add(
      CommunityVotingCreateRequested(
        communityId: widget.communityId,
        title: _titleController.text.trim(),
        tieBreakRule: _tieBreakRule,
        startsAt: now,
        endsAt: now.add(const Duration(days: 7)),
        bookIds: _selectedBooks.map((book) => book.id).toList(),
      ),
    );
    Navigator.pop(context);
  }

  @override
  Widget build(BuildContext context) {
    final theme = Theme.of(context);

    return Padding(
      padding: EdgeInsets.only(
        bottom: MediaQuery.of(context).viewInsets.bottom,
      ),
      child: SingleChildScrollView(
        child: Container(
          padding: const EdgeInsets.fromLTRB(16, 12, 16, 24),
          child: Form(
            key: _formKey,
            child: Column(
              mainAxisSize: MainAxisSize.min,
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Center(
                  child: Container(
                    width: 36,
                    height: 4,
                    decoration: BoxDecoration(
                      color: theme.colorScheme.onSurfaceVariant.withValues(
                        alpha: 0.3,
                      ),
                      borderRadius: BorderRadius.circular(2),
                    ),
                  ),
                ),
                const SizedBox(height: 16),
                Text(
                  'Nova Votação',
                  style: theme.textTheme.titleMedium?.copyWith(
                    fontWeight: FontWeight.w600,
                  ),
                ),
                const SizedBox(height: 20),
                TextFormField(
                  controller: _titleController,
                  autofocus: true,
                  maxLength: 100,
                  decoration: const InputDecoration(
                    labelText: 'Título da votação',
                    hintText: 'Ex.: Próximo livro da comunidade',
                  ),
                  validator: (value) {
                    if (value == null || value.trim().isEmpty) {
                      return 'O título é obrigatório.';
                    }
                    return null;
                  },
                ),
                const SizedBox(height: 4),
                Text(
                  'Regra de desempate',
                  style: theme.textTheme.labelMedium?.copyWith(
                    color: theme.colorScheme.onSurfaceVariant,
                  ),
                ),
                const SizedBox(height: 8),
                SegmentedButton<TieBreakRule>(
                  segments: const [
                    ButtonSegment(
                      value: TieBreakRule.adminChoice,
                      label: Text('Decisão'),
                      icon: Icon(Icons.admin_panel_settings_rounded),
                    ),
                    ButtonSegment(
                      value: TieBreakRule.randomDraw,
                      label: Text('Sorteio'),
                      icon: Icon(Icons.casino_rounded),
                    ),
                  ],
                  selected: {_tieBreakRule},
                  onSelectionChanged: (selection) {
                    setState(() => _tieBreakRule = selection.first);
                  },
                  style: ButtonStyle(
                    minimumSize: WidgetStateProperty.all(const Size(0, 44)),
                  ),
                ),
                const SizedBox(height: 20),
                Text(
                  'Obras selecionadas (${_selectedBooks.length}/6)',
                  style: theme.textTheme.labelMedium?.copyWith(
                    color: theme.colorScheme.onSurfaceVariant,
                  ),
                ),
                const SizedBox(height: 8),
                if (_selectedBooks.isEmpty)
                  Container(
                    width: double.infinity,
                    padding: const EdgeInsets.all(14),
                    decoration: BoxDecoration(
                      color: theme.colorScheme.surfaceContainerHighest,
                      borderRadius: BorderRadius.circular(12),
                    ),
                    child: const Text('Nenhuma obra selecionada ainda.'),
                  )
                else
                  Wrap(
                    spacing: 8,
                    runSpacing: 8,
                    children: _selectedBooks.map((book) {
                      return _SelectedBookTile(
                        book: book,
                        onRemove: () => _removeBook(book),
                      );
                    }).toList(),
                  ),
                const SizedBox(height: 12),
                if (_selectedBooks.length < 6)
                  _BookSearchSection(
                    controller: _bookSearchController,
                    onChanged: _onBookSearchChanged,
                    onSelect: _selectBook,
                  ),
                if (_selectedBooks.length >= 6)
                  Text(
                    'Limite de 6 obras atingido.',
                    style: theme.textTheme.bodySmall?.copyWith(
                      color: theme.colorScheme.onSurfaceVariant,
                    ),
                  ),
                const SizedBox(height: 8),
                Text(
                  'Selecione de 3 a 6 obras para publicar a votação.',
                  style: theme.textTheme.bodySmall?.copyWith(
                    color: theme.colorScheme.onSurfaceVariant,
                  ),
                ),
                const SizedBox(height: 24),
                SizedBox(
                  width: double.infinity,
                  child: FilledButton(
                    onPressed: _selectedBooks.length >= 3 ? _submit : null,
                    child: const Text('Criar rascunho'),
                  ),
                ),
              ],
            ),
          ),
        ),
      ),
    );
  }
}

class _SelectedBookTile extends StatelessWidget {
  final Book book;
  final VoidCallback onRemove;

  const _SelectedBookTile({required this.book, required this.onRemove});

  @override
  Widget build(BuildContext context) {
    return InputChip(
      label: Text(book.title),
      avatar: book.coverUrl != null
          ? ClipRRect(
              borderRadius: BorderRadius.circular(4),
              child: Image.network(
                book.coverUrl!,
                width: 24,
                height: 24,
                fit: BoxFit.cover,
                errorBuilder: (_, _, _) => const Icon(Icons.menu_book_rounded),
              ),
            )
          : const Icon(Icons.menu_book_rounded),
      onDeleted: onRemove,
    );
  }
}

class _BookSearchSection extends StatelessWidget {
  final TextEditingController controller;
  final ValueChanged<String> onChanged;
  final ValueChanged<Book> onSelect;

  const _BookSearchSection({
    required this.controller,
    required this.onChanged,
    required this.onSelect,
  });

  @override
  Widget build(BuildContext context) {
    final theme = Theme.of(context);

    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        TextField(
          controller: controller,
          onChanged: onChanged,
          decoration: const InputDecoration(
            labelText: 'Buscar obra',
            hintText: 'Buscar livro no catálogo...',
            prefixIcon: Icon(Icons.search_rounded),
          ),
        ),
        BlocBuilder<BookBloc, BookState>(
          builder: (context, state) {
            if (state is BookLoading) {
              return const Padding(
                padding: EdgeInsets.symmetric(vertical: 12),
                child: Center(child: CircularProgressIndicator()),
              );
            }
            if (state is BookLoaded) {
              final results = state.books.take(5).toList();
              return Column(
                children: results
                    .map(
                      (book) => _BookResultTile(
                        book: book,
                        onTap: () => onSelect(book),
                      ),
                    )
                    .toList(),
              );
            }
            if (state is BookEmpty) {
              return Padding(
                padding: const EdgeInsets.symmetric(vertical: 12),
                child: Text(
                  'Nenhum livro encontrado para "${state.query}".',
                  style: theme.textTheme.bodySmall,
                ),
              );
            }
            return const SizedBox.shrink();
          },
        ),
      ],
    );
  }
}

class _BookResultTile extends StatelessWidget {
  final Book book;
  final VoidCallback onTap;

  const _BookResultTile({required this.book, required this.onTap});

  @override
  Widget build(BuildContext context) {
    return ListTile(
      contentPadding: const EdgeInsets.symmetric(horizontal: 4),
      leading: book.coverUrl != null
          ? ClipRRect(
              borderRadius: BorderRadius.circular(4),
              child: Image.network(
                book.coverUrl!,
                width: 32,
                height: 46,
                fit: BoxFit.cover,
                errorBuilder: (_, _, _) => const _BookIconPlaceholder(),
              ),
            )
          : const _BookIconPlaceholder(),
      title: Text(
        book.title,
        style: Theme.of(context).textTheme.bodyMedium,
        maxLines: 1,
        overflow: TextOverflow.ellipsis,
      ),
      subtitle: Text(
        book.authorsText,
        style: Theme.of(context).textTheme.bodySmall,
        maxLines: 1,
        overflow: TextOverflow.ellipsis,
      ),
      onTap: onTap,
    );
  }
}

class _BookIconPlaceholder extends StatelessWidget {
  const _BookIconPlaceholder();

  @override
  Widget build(BuildContext context) {
    return Container(
      width: 36,
      height: 52,
      decoration: BoxDecoration(
        color: Theme.of(context).colorScheme.surfaceContainerHighest,
        borderRadius: BorderRadius.circular(4),
      ),
      child: Icon(
        Icons.menu_book_rounded,
        size: 20,
        color: Theme.of(context).colorScheme.onSurfaceVariant,
      ),
    );
  }
}
