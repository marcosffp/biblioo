...
| **Riscos:** | · **R-01 — HikariCP pool exhaustion:** picos de concorrência sustentada podem esgotar o pool de conexões JDBC compartilhado entre threads HTTP e threads de consumer RabbitMQ, causando `ConnectionTimeoutException` invisível nos logs de API. <br><br> · **R-02 — Incompatibilidade de dialeto Hibernate 6 com TiDB Cloud Serverless em lock pessimista (identificado e mitigado):** Hibernate 6 (Spring Boot 4.x) gera `SELECT ... FOR UPDATE OF alias` para queries anotadas com `@Lock(LockModeType.PESSIMISTIC_WRITE)`, onde `alias` é o alias interno gerado pelo ORM (ex.: `bvo1_0`). MySQL 8.4 é permissivo e ignora o `OF alias` em queries de tabela única, mas TiDB Cloud Serverless é estrito e interpreta `bvo1_0` como nome de tabela real, retornando `Table 'db.bvo1_0' doesn't exist` → HTTP 500 em produção. Identificado em `BookVotingOptionRepository.findByIdForUpdate()`, chamado pelo fluxo de castVote. O bug era silencioso em ambiente local (MySQL 8.4) e só se manifestava no ambiente cloud (TiDB Serverless). **Mitigação aplicada:** substituição do `@Lock(LockModeType.PESSIMISTIC_WRITE)` por native query explícita — `SELECT * FROM community_voting_options WHERE id = :id FOR UPDATE` — eliminando a geração de SQL pelo dialeto Hibernate e garantindo sintaxe válida para qualquer banco MySQL-compatible. |
| --- | --- |
...
| **Riscos:** | Risco mitigado: o Outbox Pattern garante que nenhum evento é publicado sem commit confirmado no banco. Reentregas por falha de rede são idempotentes via `event_id`, eliminando duplicatas mesmo em cenário de at-least-once delivery. Risco residual de R-01: se o pool HikariCP se esgotar, operações de escrita falham com HTTP 500 explícito ao cliente — mas o Outbox garante que nenhum evento pendente seja perdido silenciosamente. Falhas são sempre visíveis, nunca silenciosas. |
| --- | --- |
...
- **R-02 — Incompatibilidade de dialeto Hibernate 6 com TiDB Cloud Serverless (identificado e mitigado):** Hibernate 6 gerava `SELECT ... FOR UPDATE OF bvo1_0` ao processar `@Lock(LockModeType.PESSIMISTIC_WRITE)` em `BookVotingOptionRepository`, onde `bvo1_0` é o alias interno do ORM. MySQL 8.4 aceita silenciosamente essa sintaxe não-padrão em queries de tabela única; TiDB Cloud Serverless rejeita com `Table 'db.bvo1_0' doesn't exist`, causando HTTP 500 na operação de voto em produção sem qualquer evidência do problema em ambiente local. Risco inerente ao uso de banco de dados cloud com compatibilidade MySQL parcial: divergências de comportamento para SQL não-padrão só são detectadas em runtime no ambiente alvo. **Mitigação aplicada:** substituição por native query `SELECT * FROM community_voting_options WHERE id = :id FOR UPDATE`, eliminando dependência do dialeto do ORM para a geração do lock SQL.
