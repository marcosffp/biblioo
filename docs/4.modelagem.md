...
| HU-17 | Leitor | Criar uma comunidade de leitura definindo nome, descrição e visibilidade pública ou privada | Promover discussões e leitura coletiva entre leitores com interesses em comum |
...
| HU-30 | Leitor | Ter um DNA Literário gerado automaticamente com arquétipo literário dominante, temas preferidos, velocidade de leitura e distribuição de gêneros, a partir de no mínimo 5 livros lidos | Entender meu padrão de leitura de forma visual |
| HU-31 | Leitor | Receber avisos sobre eventos relevantes da plataforma, como novos seguidores e convites de comunidade | Ser informado sobre eventos relevantes de forma oportuna, sem interromper minha navegação na plataforma |
| HU-32 | Leitor | Interagir com o assistente conversacional Bibo via chat em linguagem natural para executar ações diretamente na plataforma: criar estantes, coleções e comunidades vinculadas a um livro específico, atualizar progresso de leitura, encontrar livros e autores, e obter sugestões de leitura com base no meu perfil | Realizar ações na plataforma de forma conversacional e ágil |
...
| HU-38 | Leitor | Visualizar seis trilhas de recomendação personalizadas, cada uma com critério de afinidade distinto: leituras recentes em comum com outros leitores, gêneros mais consumidos no momento, livros mais comentados nas comunidades, títulos fora do perfil habitual para ampliar horizontes, autores apreciados por leitores com gostos semelhantes e livros já lidos que podem ser revisitados com nova perspectiva | Descobrir livros alinhados ao meu gosto por diferentes perspectivas |
...
O **diagrama de componentes** representa o backend Spring Boot modularizado, os serviços de infraestrutura e as integrações externas. Os dez módulos de negócio — User, Books/Shelf/Collection, Community, Feed, Notification, Recommendation, DNA, Share, Trending e Assistant — são desacoplados entre si e se comunicam de forma assíncrona via CloudAMQP (RabbitMQ gerenciado), o que garante que falhas em um módulo não se propaguem diretamente para os demais. Os módulos Books e User consultam o Bonsai.io (OpenSearch gerenciado) para buscas de alta performance — Books para o catálogo de livros e User para perfis de leitores; Books também consome a Google Books API para enriquecer o catálogo. Os módulos Recommendation e Assistant consomem a API de LLM (Gemini) para geração de recomendações personalizadas e respostas em linguagem natural, respectivamente. O Cloudinary é responsável pelo armazenamento e entrega de imagens e GIFs enviados pelos usuários — avatares, banners, posts, comentários e mídia do chat das comunidades. Os clientes mantêm três canais de comunicação com o backend: REST/HTTPS para operações síncronas, WebSocket (STOMP) para o chat em tempo real das comunidades e SSE (Server-Sent Events) para o recebimento de notificações push sem polling.
...
5. **Cache Aside**: o Upstash (Redis gerenciado) armazena em cache resultados de consultas frequentes (ex.: feed social, histórico do assistente, sessões de comunidade), reduzindo carga no banco de dados.
6. **Escalabilidade Horizontal**: o Gateway com Cloud Load Balancing distribui a carga entre múltiplas réplicas do backend, garantindo disponibilidade e elasticidade.
...
| **REST / HTTPS** | Interface síncrona principal, consumida por Web e Mobile para todas as operações de leitura e escrita de dados. |
| **WebSocket (STOMP)** | Canal bidirecional em tempo real utilizado exclusivamente pelo módulo Community para o chat das comunidades. |
| **SSE (Server-Sent Events)** | Canal unidirecional do servidor para os clientes, utilizado pelo módulo Notification para entrega de notificações sem polling. |
| **Módulo User** | Gerencia cadastro, autenticação (e-mail/senha e Google OAuth), perfil do leitor, controle de privacidade e busca de leitores via Bonsai.io (OpenSearch). |
| **Módulo Books / Shelf / Collection** | Mantém o catálogo de livros com busca via Bonsai.io e enriquecimento pela Google Books API; controla a estante pessoal do leitor (status de leitura, progresso, avaliações) e as coleções personalizadas. |
| **Módulo Community** | Gerencia criação e participação em comunidades de leitura, incluindo chat em tempo real via WebSocket (STOMP), votações de livros, upload de mídia via Cloudinary e broadcast multi-instância via RabbitMQ. |
| **Módulo Feed** | Agrega e entrega eventos de atividade dos leitores seguidos, com fanout assíncrono via RabbitMQ e cache no Upstash (Redis). |
| **Módulo Notification** | Gera e entrega notificações ao leitor via SSE, consumindo eventos do RabbitMQ e persistindo o histórico de notificações. |
| **Módulo Recommendation** | Gera trilhas de recomendação personalizadas com base no histórico de leitura, consumindo eventos do RabbitMQ e integrando a API de LLM. |
| **Módulo DNA** | Compila o perfil analítico do leitor a partir do histórico consolidado, calculando métricas literárias com base em eventos consumidos do RabbitMQ. |
| **Módulo Assistant** | Assistente literário com IA, integrado à API de LLM (Gemini) para responder perguntas e executar ações na plataforma em linguagem natural, com histórico de conversa em cache no Upstash (Redis). |
| **Módulo Trending** | Mantém pontuações de tendência dos livros com maior engajamento, consumindo eventos do RabbitMQ para atualização contínua. |
| **Módulo Share** | Gerencia a geração e o compartilhamento da cápsula visual de leituras e a importação de bibliotecas do Goodreads. |
| **TiDB** | Banco de dados relacional distribuído compatível com MySQL, fonte de verdade para todas as entidades do domínio. |
| **Upstash** | Redis gerenciado na nuvem usado como camada de cache para feed, sessões de comunidade e histórico do assistente. |
| **CloudAMQP** | RabbitMQ gerenciado na nuvem, broker de mensagens responsável pela comunicação assíncrona entre os módulos. |
| **Bonsai.io** | OpenSearch gerenciado na nuvem, motor de busca fulltext para o catálogo de livros e para perfis de leitores. |
| **Google Books API** | Serviço externo que enriquece os dados do catálogo com metadados de livros (capa, descrição, ISBN etc.). |
| **Google OAuth** | Serviço de autenticação social do Google, consumido pelo módulo User para login com conta Google. |
| **LLM API (Gemini)** | API de IA generativa do Google, consumida pelos módulos Recommendation e Assistant para geração de recomendações e respostas em linguagem natural. |
| **Cloudinary** | Serviço gerenciado de armazenamento e entrega de imagens e GIFs — avatares, banners, posts, comentários e mídia do chat — com upload assíncrono. |
...
| **Tipo** | **Componentes** |
| --- | --- |
| **Reutilizados** | TiDB, Upstash (Redis), CloudAMQP (RabbitMQ), Bonsai.io (OpenSearch), Cloudinary, navegadores web. |
| **Adquiridos** | Google Books API, Google OAuth, LLM API (Gemini) — serviços externos consumidos via API. |
...
- **`reading_active_days`** — registra os dias em que o leitor registrou progresso de leitura (`active_date`, `book_id`, `user_id`), base para o contador de dias ativos (RF-36).
